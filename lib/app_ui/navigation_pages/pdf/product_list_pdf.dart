...
class PdfViewer extends StatefulWidget {
  ...
  PdfViewer({required this.productList});
...
class _PdfViewerState extends State<PdfViewer> {
  ...
  late pw.Font font;
  late pw.MemoryImage image;
  late PdfColor pdfColor;

  @override
  void initState() {
    super.initState();
    _future = generatePdf(widget.productList);
  }
  ...
  Future<File> generatePdf(List<Product> productList) async {
    ...
    pdfColor = PdfColor.fromInt(Colors.redAccent.value);

    pdf.addPage(pw.MultiPage(
        build: (context) => [
              buildHeader(),
              //  buildTitle(productList),
  ...
  pw.Table buildProductTable(List<Product> productList) {
    return pw.Table(
      //  border: pw.TableBorder.all(),
      columnWidths: {
        0: const pw.FlexColumnWidth(3),
            ...
            children: [
              ElevatedButton(
                onPressed: () async {
                  ...
                  final savedFile = File(path);
                  await savedFile
                      .writeAsBytes(await snapshot.data!.readAsBytes());
                  // Open the saved PDF file
                  OpenFile.open(path);
                },
